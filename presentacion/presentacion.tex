...
\begin{frame}{Notación: Parámetros del Problema}
  \begin{center}
  \renewcommand{\arraystretch}{1.25}
  \begin{tabular}{|c|l|}
    \hline
    \multicolumn{2}{|c|}{\textbf{Tabla de parámetros}} \\
    \hline
    $C_i$       & Costo de construir estación en ubicación $i$ \\
    $R$         & Distancia de autonomía promedio de autos eléctricos \\
    $d(i,j)$    & Distancia entre nodos $i,\,j$ \\
    $s_i$       & Rango de servicio de nodo $i$ \\
    $D_i$       & Demanda en el nodo $i$ \\
    $f_i$       & Poder de carga en el nodo $i$ \\
    $CityArea$  & Área de la ciudad que se debe cubrir \\
    \hline
  \end{tabular}
  \end{center}
\end{frame}
...
\begin{frame}{Modelo Matemático \gris{(Ou et al., 2025)}}
  \begin{columns}
    ...
    \textbf{Sujeto a 4 restricciones:}
    \vspace{0.5em}
    \begin{itemize}
    ...
    \end{itemize}
    \vspace{0.6em}
    {\footnotesize\gris{Cada una se desarrolla a continuación con su modelo matemático e implementación.}}
  \end{columns}
\end{frame}
...
\begin{frame}{Manejo de Restricciones: Penalidades}
  \begin{columns}[T]
    ...
    \column{0.48\textwidth}
    \textbf{Las 4 penalidades implementadas:}
    \vspace{0.3em}
    \begin{itemize}\footnotesize
      \item \acento{R1} \textbf{Accesibilidad:}\\
        $+\lambda$ por cada par $(i,j)$ sin cobertura mutua
      \vspace{0.25em}
      \item \acento{R2} \textbf{Demanda:}\\
        $+\lambda\cdot(D_j - \text{cap}_j)$ por déficit de demanda
      \vspace{0.25em}
      \item \acento{R3} \textbf{Cobertura urbana:}\\
        $+\lambda\cdot(\textit{CityArea} - \text{cobertura})$
      \vspace{0.25em}
      \item \acento{R4} \textbf{Zonas remotas:}\\
        $+\lambda$ si ninguna zona remota tiene estación
    \end{itemize}
  \end{columns}
\end{frame}

\begin{frame}[fragile]{Restricciones R1 y R2: Modelo e Implementación}
  \setlength\abovedisplayskip{3pt}\setlength\belowdisplayskip{3pt}
  \begin{columns}[T]
    \column{0.50\textwidth}
    \acento{R1} \textbf{Accesibilidad}\\
    {\footnotesize Todo par de nodos a distancia válida debe tener al menos una estación.}
    \[ x_i + x_j \geq 1 \quad \forall(i,j)\in\bar{E} \]
 ...
 for (int j=i+1; j<n; ++j)
  if (instance->isValidEdge(i,j))
   if (!chromosome[i]&&!chromosome[j])
    ...
    \column{0.48\textwidth}
    \acento{R2} \textbf{Demanda}\\
    {\footnotesize Estaciones en la vecindad $\alpha R$ del nodo $j$ deben cubrir su demanda $D_j$.}
    \[ \sum_{i\in V_j^{\alpha R}} f_i\,x_i \geq D_j \]
    \begin{lstlisting}[numbers=none]
for (int j=0; j<n; ++j) {
 double cap = 0;
 for (int i=0; i<n; ++i)
  if ((i==j || instance->dist_matrix[i][j]
      <= instance->alpha*instance->R)
      && chromosome[i])
...
\begin{frame}[fragile]{Restricciones R3 y R4: Modelo e Implementación}
  \setlength\abovedisplayskip{3pt}\setlength\belowdisplayskip{3pt}
  \begin{columns}[T]
    \column{0.50\textwidth}
    \acento{R3} \textbf{Cobertura Urbana}\\
    {\footnotesize Los rangos de servicio de las estaciones deben sumar al menos \textit{CityArea}.}
    \[ \sum_{j} s_j\,x_j \geq \textit{CityArea} \]
    \begin{lstlisting}[numbers=none]
double cov = 0;
for (int j=0; j<n; ++j)
 if (chromosome[j])
  cov += instance->nodes[j].service_range;
if (cov < instance->city_area)
 penalty += W*(instance->city_area-cov);
    \end{lstlisting}

    \column{0.48\textwidth}
    \acento{R4} \textbf{Zonas Remotas}\\
    {\footnotesize Al menos una estación en el conjunto de zonas remotas $T$.}
    \[ \sum_{j\in T} x_j \geq 1 \]
    \begin{lstlisting}[numbers=none]
int remote = 0;
for (int j=0; j<n; ++j)
 if (instance->nodes[j].is_remote
     && chromosome[j])
  remote++;
if (remote < 1)
 penalty += W;
...
\begin{frame}[fragile]{Solución Inicial: Generación Aleatoria}
  \begin{columns}[T]
    ...
    \column{0.48\textwidth}
    \textbf{Línea clave} \gris{(\texttt{Solution::randomize})}:
    \vspace{0.3em}
    \begin{lstlisting}[numbers=none]
// bit aleatorio 0 o 1 por cada nodo
chromosome[i] = bit(rng);
    \end{lstlisting}
    \vspace{0.4em}
    \begin{block}{Generador \texttt{mt19937}}
      \footnotesize Mersenne Twister: alta calidad estadística, semilla basada en el reloj del sistema para resultados distintos en cada ejecución.
    \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Movimiento I: Selección por Ruleta}
  ...
  \begin{columns}[T]
    ...
    \textbf{Línea clave} \gris{(\texttt{rouletteWheelSelection})}:
    \vspace{0.3em}
    \begin{lstlisting}[numbers=none]
// fitness bajo -> inv alto -> mayor sector
inv[i] = (max_f - pop[i].fitness) + 1.0;
    \end{lstlisting}
    \vspace{0.4em}
    \begin{block}{¿Por qué \texttt{+1.0}?}
      \footnotesize Evita que el mejor individuo quede con $\tilde{f}_i=0$ y probabilidad de selección nula.
    \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Movimiento II: Cruce Uniforme}
  ...
  \begin{columns}[T]
    \column{0.52\textwidth}
    \textbf{Línea clave} \gris{(\texttt{uniformCrossover})}:
    \begin{lstlisting}[numbers=none]
// por cada gen, moneda 50/50:
if (dist(rng) < 0.5) // intercambiar
  child1[i] = parent2[i];
else                  // heredar directo
  child1[i] = parent1[i];
    \end{lstlisting}
    \column{0.46\textwidth}
    \begin{block}{¿Por qué uniforme?}
      \footnotesize Cualquier combinación de genes de ambos padres es posible. Sin sesgo posicional a diferencia del cruce de 1 punto.
    \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Movimiento III: Mutación Bit-flip y Elitismo}
  \begin{columns}[T]
    ...
    \end{tikzpicture}
    \end{center}
    \textbf{Línea clave} \gris{(\texttt{bitFlipMutation})}:
    \begin{lstlisting}[numbers=none]
if (dist(rng) < mutation_rate) // 5%
...
new_population.push_back(best_current);
    \end{lstlisting}
    \vspace{0.4em}
    \textbf{Garantía:} el fitness global \textbf{nunca empeora} entre generaciones.
    \vspace{0.4em}
    \begin{block}{Orden en cada generación}
      \footnotesize
      Elitismo $\to$ Ruleta $\times 2$ $\to$ Cruce $\to$ Bit-flip $\to$ Evaluar
    \end{block}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Salida del Programa}
  \begin{columns}[T]
    \column{0.50\textwidth}
    {\small\textbf{Código de salida (\texttt{main.cpp}):}}
    \begin{lstlisting}
...
cout << endl;
int built = 0;
for (int x : ga.best_solution.chromosome)
  built += x;
cout << "Nodos construidos: " << built
     << "/" << instance.n_nodes << endl;
    \end{lstlisting}

    \column{0.48\textwidth}
    {\small\textbf{Compilación y ejecución:}}
    \begin{lstlisting}[language=bash,numbers=none]
$ make
$ ./evcs_solver EVCS_Instancias/\
  Instancias_pequenas/evcsSmall_N10_A0_S51.txt
    \end{lstlisting}
    {\small\textbf{Ejemplo de salida (N=10, instancia S51):}}
    \begin{lstlisting}[language={},numbers=none]
========================================
...
1, 2, 3, 4, 5, 6, 7, 8, 9
Nodos construidos: 9/10
========================================
    \end{lstlisting}
    {\scriptsize\acento{\textbf{Estado:}} \textbf{Penalidad 0} (factible) en instancias pequeñas.}
  \end{columns}
\end{frame}
...
\begin{frame}{Conclusiones y Trabajo Futuro}
  \begin{columns}[T]
    ...
    \begin{itemize}\footnotesize
      ...
      \item[\acento{\checkmark}] Soluciones factibles en instancias pequeñas ($N{=}6,8,10$)
      \item[\acento{\checkmark}] R2 con vecindad $V_j^{\alpha R}$ correcta para cualquier $\alpha$
    \end{itemize}

    \acentob{Desafío central}
    \begin{itemize}\footnotesize
      \item NP-duro: sin garantía de óptimo global
    ...
    \begin{itemize}\footnotesize
      ...
      \item[\acentob{$\to$}] Variar $\alpha$: con $\alpha{=}0$ la R1 obliga a construir en casi todos los nodos; aumentarlo reduce aristas válidas y da más flexibilidad al GA
    \end{itemize}
  \end{columns}
\end{frame}
